    ...
    class MainForm : Form
    {
        ...
        void MouseWheelControl(object sender, MouseEventArgs e)
        {
            ...
            else
            {
                ...
                if (fNumber + (FILE_RANGE * msFlag) >= 0 && fNumber + (FILE_RANGE * msFlag) < filesList.Count)
                {
                    if (this.Contains(pbList[fNumber + (FILE_RANGE * msFlag)]) == false)
                    {
                        Img_Add();
                    }
                }
            ...
            for (int i = fNumberM; i <= fNumberP; i++)
            {
                if (i >= 0 && i < filesList.Count)
                {
                    ...
                    if (this.Contains(pbList[i]) == true)
                    {
                        ...
                        pbList[i].Image = null;
                        Controls.Remove(pbList[i]);
                        Console.WriteLine($"pbList[{i}] removed!");
                    }
                    fPath = filesList[i];
        ...
        void Img_Add()
        {
            int i = fNumber + (FILE_RANGE * msFlag);
            if (i >= 0 && i < filesList.Count)
            {
                fPath = filesList[i];
                Console.WriteLine($"{fPath}");
